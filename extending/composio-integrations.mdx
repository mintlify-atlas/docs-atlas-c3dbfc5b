...
- **Prefer `ExecuteTool` over `ProgrammaticToolCalling`** — `ExecuteTool` handles auth context, error formatting, and field extraction automatically. Only drop down to `ProgrammaticToolCalling` (i.e., direct `composio.tools.execute(...)` calls) when you need conditional logic or data transformation between multiple tool calls.
